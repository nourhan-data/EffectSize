function [GlobalParams,GlobalMin] = COA_CEC2013(VarMin, VarMax, nfevalMAX,fhd,fid)
%% ------------------------------------------------------------------------
...
for c=1:pop_total
    costs(c,1) = feval(fhd,coyotes(c,:)',fid); 
end
nfeval = pop_total;
...
while nfeval<nfevalMAX % Stopping criteria
    ...
    for p=1:n_packs
        ...
        for c=1:n_coy_aux
            ...
            new_cost = feval(fhd,new_coyotes(c,:)',fid); 
            nfeval   = nfeval+1;
